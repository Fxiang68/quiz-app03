...
def show_quiz():
    ...
    if st.session_state.feedback:
        # 使用成功或錯誤樣式顯示
        if st.session_state.feedback.startswith("答對"):
    ...
    if st.button("提交答案"):
        ...
        if choice == correct:
            ...
            st.session_state.feedback = (
                f"答錯了！正確答案：{correct + 1}. {q['options'][correct]}"
            )
        # 檢查是否為最後一題
...
def show_result():
    ...
    st.success("測驗結束！")
    st.write(
        f"總共答對 {score} 題，共 {total} 題，正確率 {score / total * 100:.1f}%"
    )
    if st.button("再試一次"):
        st.session_state.quiz_started = False
        # 重置其他狀態
