...
def run_quiz(questions):
    ...
    for idx, q in enumerate(questions[:num_questions], start=1):
        ...
        if num_opts == 0:
            ...
            continue
        while True:
            ans = input(f"你的答案（輸入 1-{num_opts}）：").strip()
            # 僅接受數字並且在有效範圍內
            if ans.isdigit() and 1 <= int(ans) <= num_opts:
                break
            print(f"請輸入 1 到 {num_opts} 之間的數字。")
        # 正確答案的索引值以 0 為基準
        correct = q.get('answer_index', 0) + 1
        # 避免 answer_index 超出範圍
        if correct < 1 or correct > num_opts:
            print("題目的正確答案索引與選項數不符，視為答錯。")
            continue
        if int(ans) == correct:
            print("答對了！")
            score += 1
            ...
            # 顯示正確答案時需檢查索引範圍
            correct_text = options[correct - 1] if correct - 1 < len(options) else "<未知選項>"
            print(f"答錯了！正確答案是 {correct}: {correct_text}")
    # 顯示結果
    print("\n測驗結束！")
