...
def main():
    ...
    with st.sidebar:
        ...
        # 預設題數：不超過 10 或該冊全部題數
        default_questions = min(10, max_questions) if max_questions > 0 else 1
        # 使用下拉選單 (selectbox) 來選擇練習題數
        question_count_options = list(range(1, max_questions + 1)) if max_questions > 0 else [1]
        default_index = default_questions - 1 if default_questions >= 1 else 0
        question_count = st.selectbox(
            "選擇題數", options=question_count_options, index=default_index
        )
        # 顯示目前題庫總題數
